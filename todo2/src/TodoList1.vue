...
export default {
    ...
    if (confirm("삭제하시겠습니까?")) {
        ...
        axios.delete("http://localhost:8088/todo/delete?idlist=" + idlist.join(","))
            .then(() => this.reload())
            .catch(error => this.onError(error));
    }
},
...
input[type=text], input[text=date]{
    height:2.4em; padding : 4px; margin-right: 5px;
}
button { padding : 0.4em 1.5em; margin-right: 5px;}
table { border-collapse: collapse; margin-bottom: 20px;}
tr:nth-child(1){ background-color: #ddd;}
td { padding : 5px; border: 1px solid #aaa;}
td.checkbox{width: 50px;}
td.descripion{width : 300px;}
td.date {width : 100px;}
